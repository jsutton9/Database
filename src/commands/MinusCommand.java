...
/**
 * This outputs a table containing the elements of one table 
 * which are not in another table.
 */
public class MinusCommand extends AbstractQueryCommand {
	private Pattern pattern = Pattern.compile(
			"minus\\s+(?:([a-zA-Z]+|\\$\\d+)|\\(.*\\))\\s+and\\s+(?:([a-zA-Z]+|\\$\\d+)|\\(.*\\))\\s*;", Pattern.CASE_INSENSITIVE);
