...
/**
 * This outputs a table containing the fields and elements of 
 * two tables.
 */
public class JoinCommand extends AbstractQueryCommand {
	private Pattern pattern = Pattern.compile(
			"join\\s+(?:([a-zA-Z]+|\\$\\d+)|\\(.*\\))\\s+and\\s+(?:([a-zA-Z]+|\\$\\d+)|\\(.*\\))\\s*;", Pattern.CASE_INSENSITIVE);
